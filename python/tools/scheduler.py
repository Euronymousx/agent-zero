...
class SchedulerTool(Tool):
    ...
    async def find_task_by_name(self, **kwargs) -> Response:
        ...
        return Response(
            message=json.dumps([serialize_task(task) for task in tasks], indent=4), break_loop=False
        )
    ...
    async def create_scheduled_task(self, **kwargs) -> Response:
        ...
        if not re.match(cron_regex, task_schedule.to_crontab()):
            return Response(
                message="Invalid cron expression: " + task_schedule.to_crontab(), break_loop=False
            )
